...
export default function Marketplace() {
  ...
  const handleFilter = (arg) => {
    if (arg === 'vendor') {
      setDisplayFilter([...products.products.rows].filter(item => item.status === 'approved'))
      setPage(0)
    } else {
      setDisplayFilter([]) 
    }
  ...
  const onSortClick = (arg) => {
    const data = ((search.length < 1 && displayFilter.length === 0)
      ? [...products.products.rows].filter(item => item.status === 'approved')
          ...
          >
            <TableContainer className={classes.tContainer} component="div" >
                ...
                <TableBody>
...

                  {
                    ((search.length < 1 && displayFilter.length === 0) 
                      ...
                      // .filter(item => !search || item.category.toLowerCase().includes(search))
                      .map((item, i) => (
                        <Fragment key={i}>
                              ...
                              <TableCell className={classes.tableCell} size="small">
                                ...
                                >
                                  ...
                                  >
                                    ...
                                    >
                                      <img src={JSON.parse(item.varieties)[0].images[0]} alt="item-logo" className={classes.image} />
                                    </Box>
                                  </a>
                                </Link>
                              </TableCell>
              ...
              <TablePagination
                ...
                count={
                  (search.length < 1 && displayFilter.length === 0)
                      ? products.products.filter(item => item.status === 'approved').length
