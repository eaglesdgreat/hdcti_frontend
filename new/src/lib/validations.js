...
const validations = (value, name, required = true, type, secondValue) => { 
    ...
    // validation for length of name feild
    if (type === "name" && !nameRegex.test(value)) {
      return { message: `${name} can not be a single alphabet`, status: true };
    }
