  ...
  >
    <div class="mb-12" style="display:flex; flex-direction:column;">
        ...
        <div style="display:flex; width:100%; flex-direction:column;">
          ...
          >
            ...
            <v-progress-linear
              v-else
              height="10"
              :value="Math.round((4 / 6) * 100)"
              striped
            >
              {{ Math.round((4 / 6) * 100) }}% completion
            </v-progress-linear>
          </div>
        </div>
      </div>
      ...
      <div style="display:flex; width:40%; margin:auto;">
        <p class="text-input4" style="display:flex; justify-content:center;">
          Set-up your business units
        </p>
      </div>

      <div style="display:flex; width:60%; margin:auto; padding-top:20px;">
        <p class="note-text4">
          Business units in Stride helps to categorize your organization into
          sections and will be used to group employees and activities.
        </p>
      </div>
              ...
              <v-container>
                <v-row>
                  <v-col cols="12" sm="12" md="12" lg="12">
                    ...
                    >
                      <p class="text-style4">BUSINESS UNIT:</p>
                    </div>

                    <v-text-field
                      v-model="business_unit"
                      placeholder="Enter your business units"
                      outlined
                      height="45"
                      type="text"
                    ></v-text-field>
...
export default {
  ...
  methods: {
    ...
    getProjects(url = "http://opsmanager.local/api/get/divisions") {
      // this.show = true;
      this.tableData.draw++;
      axios
    ...
    async handleSubmit(event) {
      ...
      // console.log(body)
      const url = `http://opsmanager.local/api/divisions`;
    ...
    async sendEditData() {
      ...
      if(isValid) {
        try {
          const update =  await axios.post('/api/edit/divisions/'+this.editdivision.id, body)
          console.log(update)
        } catch(e) {
          console.log(e)
        }
